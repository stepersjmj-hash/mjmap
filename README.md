...
- ☕ **대형카페** — KakaoMap·NaverMap "대형카페" 검색 결과 정적 JSON (전국 4,604건), 메뉴·좌석·영업시간·주차·태그 상세 패널 표시
...
[브라우저 — index.html + js/]
       ...
       │                (대형카페): js/cafe/large_cafes.json (4,604건, Kakao/Naver 통합 — 좌표 x/y, 썸네일 thumbnail, 외부링크 url)
       │
       └─→ CORS 프록시 — proxy.py
              │
...
- **선택한 마커 활성화 표시** — 정보창 또는 자세히보기 패널이 열린 동안, 해당 마커를 **색상 반전**(흰색 배경 + 카테고리 색 아이콘/테두리)으로 강조 (클러스터/즐겨찾기 마커 포함). 닫으면 원래대로 복귀
